...
import {getAuthHeader} from '../../utils/toast'
...
export const getArticles=(sort)=>{
    return async(dispatch,getState)=>{
        try{
            ...
            dispatch(articles.successGlobal('Loading successfull'))
        }catch(error){
...
export const articleStatus=(status,_id)=>{
    return async(dispatch,getState)=>{
        try{
            ...
            dispatch(articles.articleStatus(state))
            dispatch(articles.successGlobal('Updated'))
        }catch(error){
...
export const getPaginateArticles=(page=1,limit=5)=>{
    return async(dispatch)=>{
        try{
            const request=await axios.post(`/api/articles/admin/paginator`,{
                page,limit
            },getAuthHeader())
...
export const addArticle=(article)=>{
    return async(dispatch)=>{
        try{
            const request=await axios.post(`/api/articles/admin/addarticle`,article,getAuthHeader())
            dispatch(articles.addArticle(request.data))
            dispatch(articles.successGlobal('Good to go'))
        }catch(error){
            dispatch(articles.errorGlobal(error.response.data.message))
        }
    }
...
export const removeArticle=(id)=>{
    return async(dispatch)=>{
        try{
            ...
            dispatch(articles.errorGlobal(error.respomse.data.message))
        }
    }
}
...
export const articleUpdate=(article,id)=>{
    return async(dispatch)=>{
        try{
            ...
            dispatch(articles.successGlobal('Updated Successfuly'))
        }catch(error){
            dispatch(articles.errorGlobal('Something went wrong'))
        }
    }
}